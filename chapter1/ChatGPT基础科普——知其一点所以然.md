# ChatGPT基础科普——知其一点所以然
大部分人对大模型的原理理解都是错误的。他们是这样理解的：GPT 这种大模型就是对历史数据做了大量的概率统计,从而获得预测下一个字的能力。
这种预测是历史的重复，实际上这是非常肤浅的。很抱歉我起初我也是这么理解的。

举一个简单的例子：你给ChatGPT 输入一句话，假设这句话从来没有在历史上出现过，你会发现 ChatGPT 不但给出了输出，而且还很符合逻辑。这是为什么呢？

实际上大模型的训练结果就是训练出了一条公式，这条公式有很多参数，你的输入就是变量、这些变量和这些参数做了运算后得到的就是下一个字（token）。当我们用小规模的历史文本去训练这条公式的时候，模型的输出质量很一般，但当不断的增加这些训练文本的规模，惊讶的发现这个模型变得非常神奇、它似乎理解了人类的逻辑，即使没有见过的东西，它也能准确预测下一个字、它变得真正的智能起来。

至于为什么会这样，至今还是一个谜...。就好像我们至今也没有搞懂人类为什么会有意识。
...
![](images/paper-attention-is-all-you-need-1.jpeg)
...
* \- 去除：举例，`java +面试 -培训`
* "" 强加于搜索(避免忽略词)：举例，`"面试的艺术"`
* 或：举例，搜索 Java 或 Python 面试题 => `(java | python) +面试` 
* related：举例，`related:google.com`
* intitle：举例，`面试题 allintitle:jvm`
* inurl：举例，`java inurl:executors`
* filetype：举例，`spring boot (filetype:doc | filetype:xls | filetype:ppt)`
* site：举例，`java tutorial site:oracle.com` 
